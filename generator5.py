...
def generate_single_card(row, template_path, font):
    ...
    try:
        ...
        # Draw white rectangles to cover placeholders
        # for field in field_names:
        #     if field in coordinates and field in text_sizes:
...
def generate_admit_cards():
    ...
    if ENABLE_MULTITHREADING:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {executor.submit(generate_single_card, row, Imagefilename, font): row 
...
def generate_pdf_output():
    ...
    try:
        ...
        for _, row in data.iterrows():
            safe_filename = str(row['roll no.']).replace('/', '_').replace('\\', '_')
            img_path = os.path.join(Output_path, f"{safe_filename}.png")
...
if __name__ == "__main__":
    ...
    if user_input == "Load":
        ...
        print("Invalid input. Please type 'Load', 'Preview', 'Generate', or 'PDF'")
        exit(1)
